    ...
    public void processOrder(final OrderDto order, final Long userId)
        ...
        if (orderId < 0) {
            ...
            for (ItemDto orderItem :order.getItems()) {
                LOGGER.info("Adding item  "+orderItem.getProductId()+",  "+orderItem.getQuanntity()+"  pos");
                shopService.addItem(orderId, orderItem.getProductId(), orderItem.getQuanntity());
            }
            ...
            if (wasError) {
                LOGGER.info("Canecelling order  "+orderId);
